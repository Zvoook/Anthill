...
protected:
    ...
    float radius_vision;
public:
    Entity(float x = 0, float y = 0, float radius = ant_size, Color color = Color::White) : pos(x, y), velocity(0, 0), age(0), radius_vision(80.f),has_target(false), target(0, 0), visible(true)
    {
        hp = rand() % 201 + 300;
